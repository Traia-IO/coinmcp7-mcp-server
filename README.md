...
This is an MCP (Model Context Protocol) server that provides with authentication via Bearer tokens access to the coinmcp7 API. It enables AI agents and LLMs to interact with coinmcp7 through standardized tools.

## Features

- 🔧 **MCP Protocol**: Built on the Model Context Protocol for seamless AI integration
- 🌐 **Full API Access**: Provides tools for interacting with coinmcp7 endpoints
...
- **coinmcp7 Website**: [https://pro-api.coinmarketcap.com/](https://pro-api.coinmarketcap.com/)
- **API Documentation**: []()
...
*Note: Replace `example_tool` with actual coinmcp7 API tools based on the documentation.*
...
1. Clone this repository:
   ```bash
   git clone https://github.com/Traia-IO/coinmcp7-mcp-server.git
   cd coinmcp7-mcp-server
   ```

2. Set your API key:
   ```bash
   export COINMCP7_API_KEY="your-api-key-here"
   ```
...
# Server's internal API key (for payment mode)
   COINMCP7_API_KEY=your-api-key-here
...
2. Run the server:
   ```bash
COINMCP7_API_KEY="your-api-key-here" uv run python -m server
   ```
...
Clients with their own coinmcp7 API key can use the server for free:

```bash
# Request with client's API key
curl -X POST http://localhost:8000/mcp \
  -H "Authorization: Bearer CLIENT_COINMCP7_API_KEY" \
  -H "Content-Type: application/json" \
  -d '{"method":"tools/call","params":{"name":"example_tool","arguments":{"query":"test"}}}'
```

**Flow**:
1. Client provides their coinmcp7 API key
2. Server uses client's API key to call coinmcp7 API
3. No payment required
4. Client pays coinmcp7 directly
...
6. Server uses its INTERNAL coinmcp7 API key to call the API
...
```bash
# Required
COINMCP7_API_KEY=your_internal_coinmcp7_api_key  # Server's API key (for payment mode)
...
1. **Client Decision**:
   - Has coinmcp7 API key? → Mode 1 (Authenticated)
...
1. Create API client functions for coinmcp7 endpoints
...
```json
{
  "github_url": "https://github.com/Traia-IO/coinmcp7-mcp-server",
  "mcp_server": {
    "name": "coinmcp7-mcp",
    "description": "Coinmcp7 coinmcp7 coinmcp7 coinmcp7 coinmcp7 coinmcp7 coinmcp7 ",
...
- `COINMCP7_API_KEY`: Your coinmcp7 API key (required)
## Troubleshooting
