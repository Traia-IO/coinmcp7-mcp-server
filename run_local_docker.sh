...
# Script to build and run the coinmcp7 MCP Server locally in Docker
...
IMAGE_NAME="coinmcp7-mcp-server"
CONTAINER_NAME="coinmcp7-mcp-local"
HOST_PORT=${PORT:-8000}
CONTAINER_PORT=${PORT:-8000}

echo -e "${BLUE}🚀 Building and running coinmcp7 MCP Server...${NC}"
echo
...
if [ ! -f .env ]; then
    ...
    if [ -f .env.example ]; then
        ...
        cat > .env << EOF
# coinmcp7 MCP Server Configuration
PORT=8000
STAGE=MAINNET
LOG_LEVEL=INFO
...
if ! grep -q "^COINMCP7_API_KEY=[^[:space:]]" .env; then
    if [ -t 0 ]; then
        echo ""
        echo -e "${YELLOW}📝 Please enter your coinmcp7 API key:${NC}"
        echo -e "${YELLOW}   (This is the server's internal API key used when clients pay via 402)${NC}"
        read -p "COINMCP7_API_KEY: " API_KEY_VALUE
        if [ -n "$API_KEY_VALUE" ]; then
            if grep -q "^COINMCP7_API_KEY=" .env; then
                if [[ "$OSTYPE" == "darwin"* ]]; then
                    sed -i '' "s|^COINMCP7_API_KEY=.*|COINMCP7_API_KEY=$API_KEY_VALUE|" .env
                else
                    sed -i "s|^COINMCP7_API_KEY=.*|COINMCP7_API_KEY=$API_KEY_VALUE|" .env
                fi
            else
                echo "COINMCP7_API_KEY=$API_KEY_VALUE" >> .env
            fi
        ...
        echo -e "${YELLOW}⚠️  Non-interactive mode: Please set COINMCP7_API_KEY in .env manually${NC}"
    fi
else
    echo -e "${GREEN}✅ COINMCP7_API_KEY already set in .env${NC}"
fi
...
echo
echo -e "${GREEN}✅ coinmcp7 MCP Server is running!${NC}"
echo
