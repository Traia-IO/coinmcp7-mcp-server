# coinmcp7 MCP Server Implementation Guide

You are working on implementing the coinmcp7 MCP (Model Context Protocol) server. The basic structure has been set up, and your task is to implement the actual API integration.
...
- **Authentication**: Required - API key via `COINMCP7_API_KEY` environment variable
...
When a client provides their own coinmcp7 API key:
...
Flow:
  1. Client connects with their coinmcp7 API key
  2. MCP server uses client's API key to call coinmcp7 API
  3. No payment required
  4. Client pays coinmcp7 directly (not the MCP server)
```

**Use Case**: Clients who already have a coinmcp7 subscription/API key
...
Flow:
  ...
  4. MCP server uses its INTERNAL coinmcp7 API key to call the API
  5. Client pays the MCP server (not coinmcp7)
```

**Use Case**: Pay-per-use clients without their own coinmcp7 subscription
...
async def your_tool_name(context: Context, param1: str) -> Dict[str, Any]:
    ...
    if api_key:
        ...
        api_key_to_use = os.getenv("COINMCP7_API_KEY")
...
**Required**:
- `COINMCP7_API_KEY`: Server's internal coinmcp7 API key (used when clients pay via 402)
...
COINMCP7_API_KEY=your_coinmcp7_api_key_here
...
```json
{
  ...
  "tags": ["coinmcp7", "api", /* add relevant tags like "search", "data", etc. */]
}
```
...
Add functions to call the coinmcp7 API with retry support. Example pattern:
...
def call_coinmcp7_api(endpoint: str, params: Dict[str, Any], api_key: str) -> Dict[str, Any]:
    """Call coinmcp7 API endpoint with automatic retry."""
...
"Content-Type": "application/json"
    ...
    try:
        ...
        raise Exception(f"coinmcp7 API error: {str(e)}")
```
...
```python
@mcp.tool()
async def search_coinmcp7(
    context: Context,
    query: str,
    limit: int = 10
) -> Dict[str, Any]:
    """
    Search coinmcp7 for [specific data type].
    ...
    try:
        # The call_coinmcp7_api function already has retry logic
        results = call_coinmcp7_api(
...
```python
@mcp.tool()
async def create_coinmcp7_item(
    context: Context,
    name: str,
    data: Dict[str, Any]
) -> Dict[str, Any]:
    """
    Create a new item in coinmcp7.
...
3. **Add Tool Examples** in README.md:
   ...
   result = await tool.search_coinmcp7(query="example", limit=5)
   ```
...
- Check the coinmcp7 API documentation: 
- Review the MCP specification: https://modelcontextprotocol.io
- Look at other MCP server examples in the Traia-IO organization

Remember: The goal is to make coinmcp7's capabilities easily accessible to AI agents through standardized MCP tools. 